...
idRDV: 2,
  ...
  getrendezvous_patient(patiendid:number):RendezvousModel[]
    {
    return this.rendezvous.filter(rdv => rdv.patientId === patiendid);
    }
  // methode pour ajouter un rendezvous
  addrendezvous(newrendezvous: RendezvousModel): void 
    {
      newrendezvous.idRDV = this.rendezvous.length + 1;
      this.rendezvous.push(newrendezvous);
    }
    annulerRendezvous(idrendezvous: number): void 
      {
        const rd = this.rendezvous. find(rd=> rd.idRDV===idrendezvous);
        if(rd){
          rd.statut='annulé';
        }
      }
